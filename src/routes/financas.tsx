...
function Financas() {
  const t = useT();
  const { transactions, addTransaction, updateTransaction, removeTransaction, addCustomItem, customTaxonomy } = useStore();
        ...
        <GlassCard>
          ...
          <div className="max-h-[400px] overflow-y-auto space-y-2">
            ...
            {monthTx.map((tx) => {
              ...
              const customCat = customTaxonomy.financeCategories.find((c) => c.id === tx.category);
              const catIcon = cat?.icon || customCat?.icon || "📦";
              const catLabel = taxLabel(t, "cat.", tx.category);
              return (
                <div
                  key={tx.id}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {fmtDate(tx.date)} • {catLabel}
                    </div>
                  </div>
                  <div
                    className={`font-semibold ${tx.type === "entrada" ? "text-emerald-bgt" : "text-coral"}`}
                  >
                    {tx.type === "entrada" ? "+" : "-"}
                    {brl(tx.value)}
                  </div>
