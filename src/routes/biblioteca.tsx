...
function Biblioteca() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.author.trim()) {
      toast.error(t("biblioteca.toastRequired"));
      return;
    }
    addBook({
      title: form.title,
    ...
    toast.success(t("biblioteca.toastAdded"));
  };
  ...
  return (
    <div>
      <PageHeader
        title={t("biblioteca.pageTitle")}
        subtitle={
          books.length === 1
            ? t("biblioteca.booksReadOne", { count: books.length })
            : t("biblioteca.booksReadMany", { count: books.length })
        }
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label={t("biblioteca.kpiBooksRead")} value={books.length} icon="📚" tone="gold" />
        <KpiCard
          label={t("biblioteca.kpiBooksInYear", { year })}
          value={thisYear.length}
          icon="📖"
        />
        <KpiCard
          label={t("biblioteca.kpiAvgRating")}
          value={avgRating ? avgRating.toFixed(1) : "—"}
          icon="⭐"
          tone="gold"
        />
        <KpiCard
          label={t("biblioteca.kpiTotalPages")}
          value={totalPages.toLocaleString("pt-BR")}
          icon="📃"
        />
      </div>

      <Section title={t("biblioteca.addBook")}>
        <GlassCard>
          <form onSubmit={submit} className="grid md:grid-cols-2 gap-3">
            <input
              className={inpCls}
              placeholder={t("biblioteca.title")}
              value={form.title}
            ...
            <input
              className={inpCls}
              placeholder={t("biblioteca.author")}
              value={form.author}
            ...
            <input
              className={inpCls}
              type="number"
              placeholder={t("biblioteca.pubYear")}
              value={form.year}
            ...
            <input
              className={inpCls}
              type="number"
              placeholder={t("biblioteca.pagesNum")}
              value={form.pages}
            ...
            <textarea
              className={`${inpCls} md:col-span-2`}
              rows={3}
              placeholder={t("biblioteca.notesPlaceholder")}
              value={form.notes}
            ...
            <textarea
              className={`${inpCls} md:col-span-2`}
              rows={2}
              placeholder={t("biblioteca.applicationsPlaceholder")}
              value={form.applications}
              onChange={(e) => setForm({ ...form, applications: e.target.value })}
            />
            <button type="submit" className={`${btnGold} md:col-span-2`}>
              {t("biblioteca.addToLibrary")}
            </button>
          </form>
        </GlassCard>
      </Section>
      ...
      <Section>
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((b) => (
            <GlassCard
              key={b.id}
              className="group flex flex-col justify-between relative hover:border-gold/40 transition"
            >
              <div>
                <div
                  ...
                  style={{
                    background: `linear-gradient(135deg, ${GENRE_COLORS[b.genre] || "#888"}, ${GENRE_COLORS[b.genre] || "#888"}88)`,
                  }}
                >
                  {b.title[0]}
                ...
                {b.notes && (
                  <p className="text-xs text-muted-foreground line-clamp-2 italic">"{b.notes}"</p>
                )}
              </div>
              ...
              <div className="pt-3 mt-3 border-t border-white/5 flex items-center justify-between">
                ...
                {(b.notes || b.applications) && (
                  ...
                  >
                    <Eye size={12} /> {t("biblioteca.viewNotes")}
                  </button>
                )}
              </div>
            </GlassCard>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <GlassCard>
            <h4 className="text-sm text-muted-foreground mb-2">
              {t("biblioteca.booksByMonth", { year })}
            </h4>
          ...
          <GlassCard>
            <h4 className="text-sm text-muted-foreground mb-2">{t("biblioteca.byGenre")}</h4>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <GlassCard>
            <h4 className="text-sm text-muted-foreground mb-2">{t("biblioteca.ratingOverTime")}</h4>
            <ResponsiveContainer width="100%" height={200}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {viewingBook.genre} • {t("biblioteca.finishedOn")}{" "}
                  {fmtDate(viewingBook.finishedAt)}
                </p>
              </div>
        ...
        >
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs text-muted-foreground block mb-1">
                  {t("biblioteca.finishedDate")}
                </label>
                <input
                  name="finishedAt"
                  type="date"
                  defaultValue={editingBook.finishedAt}
                  className={inpCls}
                  required
                />
              </div>
            ...
            <div>
              ...
              <textarea
                name="applications"
                defaultValue={editingBook.applications ?? ""}
                rows={2}
                className={inpCls}
              />
            </div>
